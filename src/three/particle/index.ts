...
Pool.prototype = {
  ...
  get: function (obj) {
    var p,
      puid = obj.__puid || Proton.PUID.id(obj);
...
var Integration = function (type) {
  this.type = Proton.Util.initValue(type, EULER);
};
...
export const Vector3D = function (x, y, z) {
  this.x = x || 0;
  this.y = y || 0;
  this.z = z || 0;
};
...
Vector3D.prototype = {
  ...
  getGradient: function () {
    ...
    else if (this.y > 0) return Proton.PI / 2;
    else if (this.y < 0) return -Proton.PI / 2;
  },
...
export function Span(a, b, center) {
  this._isArray = false;
...
Span.prototype = {
  getValue: function (INT) {
    if (this._isArray) {
      ...
      else return Proton.MathUtils.randomFloating(this.a, this.b, INT);
    }
  },
};
...
Proton.createArraySpan = function (arr) {
  if (!arr) return null;
  if (arr instanceof Proton.ArraySpan) return arr;
  else return new Proton.ArraySpan(arr);
};
...
function Behaviour(life, easing) {
   ...
   */
  this.easing = Proton.Util.initValue(
    easing,
    Proton.ease.setEasingByName(Proton.ease.easeLinear)
  );
  this.age = 0;
  this.energy = 1;
  /**
...
export function Rate(numPan, timePan) {
...
export function Velocity(a, b, c) {
...
export function Force(fx, fy, fz, life, easing) {
...
function Attraction(targetPosition, force, radius, life, easing) {
  ...
  this.targetPosition = Proton.Util.initValue(
    targetPosition,
    new Proton.Vector3D()
  );
...
Attraction.prototype.reset = function (
  targetPosition,
  force,
  radius,
  life,
...
RandomDrift.prototype.reset = function (
  driftX,
  driftY,
  driftZ,
  delay,
  ...
  this.randomFoce = this.normalizeForce(
    new Proton.Vector3D(driftX, driftY, driftZ)
  );
  this.delayPan = Proton.createSpan(delay || 0.03);
  this.time = 0;
...
Repulsion.prototype.reset = function (
  targetPosition,
  force,
  radius,
  life,
...
export function Collision(emitter, useMass, callback, life, easing) {
...
export function CrossZone(a, b, life, easing) {
...
export function Alpha(a, b, life, easing) {
...
export function Scale(a, b, life, easing) {
...
Scale.prototype.applyBehaviour = function (particle, time, index) {
  ...
  particle.scale = Proton.MathUtils.lerp(
...
Rotate.prototype.initialize = function (particle) {
  switch (this._type) {
    ...
    case "add":
      particle.transform.addR = new Proton.Vector3D(
        this.a.getValue(),
        this.b.getValue(),
        this.c.getValue()
      );
...
Rotate.prototype.applyBehaviour = function (particle, time, index) {
  ...
  switch (this._type) {
    ...
    case "to":
      particle.rotation.x = Proton.MathUtils.lerp(
        particle.transform.fR.x,
        particle.transform.tR.x,
        this.energy
      );
      particle.rotation.y = Proton.MathUtils.lerp(
        particle.transform.fR.y,
        particle.transform.tR.y,
        this.energy
      );
      particle.rotation.z = Proton.MathUtils.lerp(
...
export function Color(a, b, life, easing) {
...
Color.prototype.initialize = function (particle) {
  ...
  else particle.transform.colorB = Proton.ColorUtil.getRGB(this.b.getValue());
};
...
Color.prototype.applyBehaviour = function (particle, time, index) {
  ...
  if (!this._same) {
    particle.color.r = Proton.MathUtils.lerp(
      particle.transform.colorA.r,
      particle.transform.colorB.r,
      this.energy
    );
    particle.color.g = Proton.MathUtils.lerp(
      particle.transform.colorA.g,
      particle.transform.colorB.g,
      this.energy
    );
    particle.color.b = Proton.MathUtils.lerp(
...
Spring.prototype.reset = function (x, y, z, spring, friction) {
  if (!this.pos) this.pos = new Proton.Vector3D(x, y, z);
...
export class FollowEmitter extends Emitter {
    constructor(mouseTarget?: any, ease?: any, pObj?: any) {
      super(pObj);
...
export class BoxZone extends Zone {
  ...
  getPosition() {
    this.vector.x =
      this.x + Proton.MathUtils.randomAToB(-0.5, 0.5) * this.width;
    this.vector.y =
      this.y + Proton.MathUtils.randomAToB(-0.5, 0.5) * this.height;
    this.vector.z =
      this.z + Proton.MathUtils.randomAToB(-0.5, 0.5) * this.depth;
    return this.vector;
  }
...
export function Box(x, y, z, w, h, d) {
  this.x = x;
  this.y = y;
  this.z = z;
  this.width = w;
...
export const Debug = {
  ...
  drawZone: function (proton, container, zone) {
    ...
    if (zone instanceof PointZone) {
      ...
      if (zone.geometry instanceof THREE.BufferGeometry) geometry = zone.geometry;
